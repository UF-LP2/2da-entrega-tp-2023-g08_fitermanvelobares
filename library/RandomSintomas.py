...
def EnvejecerPacientes(ListaPacientes:List[Paciente]):
    SintomasPosibles = ["No Respira", "Politraumatismo Grave", "Coma", "Convulsiones", "Hemorragia Digestiva", 
                        ...
                        "Odontalgias", "Dolores Inespecíficos Leves", "Traumatismos", "Esguinces", "hipo", 
                        "resfrio", "me duele el pie", "piojos"]
    #^todos los sinotmas que puede tener el paciente
    
    enfermeroAux = Enfermero() #creo un enfermero
    
    CantidadRdn =  random.randint(0, len(ListaPacientes)-1) #decido cuantos pacientes experimentaron nuevos sintomas (puede que terminen bajando de nivel)
